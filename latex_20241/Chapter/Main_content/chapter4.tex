...
\begin{itemize}
	\item Với các Components có thể lặp lại, chúng em có thể viết riêng ra để có thể tái sử dụng code.
...
Để giúp cho việc xây dung hệ thống thuận tiện hơn , sản phẩm của chúng em đã được hỗ trợ áp dụng quy trình CI/CD giúp việc cập nhật và triển khai dễ dàng và liên tục. Để làm được việc này thì nhóm chúng em đã sử dụng VPS để triển khai server API cũng như website. Công nghệ Docker giúp tự động deploy ứng dụng. Để quản lý cơ sở dữ liệu chúng em dùng MySQL Server và MongoDb Compass. Và cuối cùng là Github để quản lý code của dự án.
