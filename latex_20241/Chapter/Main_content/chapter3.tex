...
\begin{enumerate}[a)]
  ...
  \begin{xltabular}{\textwidth}{
    ...
    \caption{\bfseries \fontsize{12pt}{0pt}\selectfont Bảng API xác thực tài khoản}
    \label{table_api_auth}
    \\
    \hline
    \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
    api/ecg/auth/register   &   POST  & Đăng ký tài khoản \\ \hline
  ...
  \begin{xltabular}{\textwidth}{
    ...
    \caption{\bfseries \fontsize{12pt}{0pt}\selectfont Bảng API quản lý dữ liệu người dùng}
    \label{table_api_user}
    \\
    \hline
    \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
    api/ecg/users   &   GET  &  Lấy danh sách thông tin của tất cả người dùng \\  \hline
	api/ecg/users/doctors   &   GET  &  Lấy danh sách bác sĩ \\  \hline
	api/ecg/users/:id   &   GET  &  Lấy dữ liệu người dùng cụ thể \\  \hline
	api/ecg/users/data/ patient-data   &   GET  &  Lấy danh sách bệnh nhân của bác sĩ cụ thể\\  \hline
	api/ecg/users/data/ doctor-id   &   GET  &  Lấy danh sách bệnh bác sĩ phụ trách bệnh nhân cụ thể \\  \hline
	api/ecg/users/   &   PUT  &  Cập nhật thông tin người dùng \\  \hline
	api/ecg/users/:userId  &   DELETE  &  Xóa người dùng cụ thể \\  \hline
  \end{xltabular}
...
\begin{xltabular}{\textwidth}{
  ...
  \hline
  \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
  api/ecg/device   &   GET  & Lấy danh sách thiết bị \\ \hline
  api/ecg/devide/:id   &    GET    & Lấy thông tin của một thiết bị \\ \hline
  api/ecg/device/add &   POST     & Thêm thiết bị mới \\ \hline
  api/ecg/device\_detail &   POST     & Thêm chi tiết cho thiết bị \\ \hline
  api/ecg/device/:device\_id  &     DELETE   & Xóa thiết bị cụ thể \\ \hline
  api/ecg/device/:device\_id  &     PUT   & Cập nhật thông tin bị cụ thể \\ 
  api/ecg/device\_detail  &     PUT   & Cập nhật thông số kỹ thuật của thiết bị \\ \hline
  api/ecg/device\_detail/ :detail\_id  &     PUT   & Xoá thông số kỹ thuật của thiết bị \\ \hline
\end{xltabular}
...
\begin{xltabular}{\textwidth}{
  ...
  \hline
  \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
 api/ecg/records   &   GET  & Lấy tất cả dữ liệu phiên đo \\ \hline
 api/ecg/records/:id   &    GET    & Lấy dữ liệu phiên đo cụ thể \\ \hline
 api/ecg/records/doctor/ :doctorId &   GET     & Lấy danh sách các dữ liệu phiên đo của bác sĩ phụ trách cụ thể \\ \hline
 api/ecg/records/   &    POST    & Tạo mới dữ liệu phiên đo\\ \hline
 api/ecg/records/   &    PUT    & Cập nhật dữ liệu phiên đo\\ \hline
 api/ecg/records/:record\_id  &    DELETE    & Xóa dữ liệu phiên đo cụ thể\\ \hline
  \end{xltabular}


\item API đặt lịch hẹn bác sĩ - bệnh nhân
\begin{xltabular}{\textwidth}{
  | >{\raggedright\arraybackslash}m{4.5cm}
  | >{\centering\arraybackslash}m{2.8cm}
  | >{\raggedright\arraybackslash}X |
  }
  \caption{\bfseries \fontsize{12pt}{0pt}\selectfont Bảng API đặt lịch hẹn bác sĩ - bệnh nhân}
  \label{table_api_schedule}
  \\
  \hline
  \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
  api/ecg/schedules   &   GET  & Lấy danh sách tất cả lịch hẹn  \\ \hline
  api/ecg/schedules/doctor-id  &    GET    & Lấy danh sách lịch hẹn của bác sĩ cụ thể \\ \hline
  api/ecg/schedules/patient-id  &    GET    & Lấy danh sách lịch hẹn của bệnh nhân cụ thể \\ \hline
  api/ecg/schedules/create-by-doctor  &    POST    & Bác sĩ đặt lịch tái khám cho bệnh nhân \\ \hline
  api/ecg/schedules/create-by-patient  &    POST    & Bệnh nhân đặt lịch khám \\ \hline
  api/ecg/schedules/time/ available-doctor/ :schedule\_time  &    GET    & Lấy danh sách bác sĩ có thể đặt lịch hẹn theo thời gian cụ thể\\ \hline
  api/ecg/schedules/ available-schedule/:id  &    GET    & Lấy danh sách thời gian có thể đặt lịch hẹn với bác sĩ cụ thể\\ \hline
  api/ecg/schedules/ accept-schedule  &    PUT    & Chấp nhận lịch hẹn cụ thể \\ \hline
  api/ecg/schedules/reject-schedule/:id  &    DELETE    & Từ chối lịch hẹn cụ thể \\ \hline
  \end{xltabular}

  \item API liên quan đến chẩn đoán
\begin{xltabular}{\textwidth}{
  | >{\raggedright\arraybackslash}m{4.5cm}
  | >{\centering\arraybackslash}m{2.8cm}
  | >{\raggedright\arraybackslash}X |
  }
  \caption{\bfseries \fontsize{12pt}{0pt}\selectfont Bảng API liên quan đến chẩn đoán}
  \label{table_api_diagnosis}
  \\
  \hline
  \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
  api/ecg/diagnosis   &   POST  & Tạo chẩn đoán mới  \\ \hline
  api/ecg/diagnosis/ schedule/:schedule\_id   &   POST  & Lấy thông tin chẩn đoán của lịch khám cụ thể\\ \hline
  api/ecg/diagnosis/update   &   POST  & Cập nhật thông tin chẩn đoán\\ \hline
  \end{xltabular}


\item API liên quan đến thông báo
\begin{xltabular}{\textwidth}{
  | >{\raggedright\arraybackslash}m{4.5cm}
  | >{\centering\arraybackslash}m{2.8cm}
  | >{\raggedright\arraybackslash}X |
  }
  \caption{\bfseries \fontsize{12pt}{0pt}\selectfont Bảng API liên quan đến thông báo}
  \label{table_api_notification}
  \\
  \hline
  \bfseries Đường dẫn    &\bfseries Phương thức    &\bfseries Mô tả\\ \hline
  api/ecg/notification/get   &   GET  & Lấy thông báo của người dùng cụ thể  \\ \hline
  api/ecg/notification   &   POST  & Tạo thông báo mới\\ \hline
  api/ecg/notification/ update-seen   &   POST  & Cập nhật trạng thái thông báo đã được xem\\ \hline
  api/ecg/notification/:id   &   DELETE  & Xóa thông báo cụ thể\\ \hline
  \end{xltabular}
...
\begin{xltabular}{\textwidth}{
  ...
  api/ecg/groupChat   &   GET  & Lấy danh sách nhóm trò chuyện của người dùng  \\ \hline
  api/ecg/chat/messages/ :groupChatId   &   GET  & Lấy danh sách tin nhắn theo nhóm trò chuyện\\ \hline
  api/ecg/chat/send   &   POST  & Gửi tin nhắn trong nhóm trò chuyện\\ \hline
  \end{xltabular}
\end{enumerate}
