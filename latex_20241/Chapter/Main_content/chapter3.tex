...
\end{figure}
Hệ thống bao gồm ba thành phần chính: Máy chủ (Server), Thiết bị (Device) và Ứng dụng (bao gồm web và app).
...
Hình \ref{device-info-admin} là giao diện cửa sổ trượt khi click vào 1 thiết bị bất kỳ nói trên. Nội dung chi tiết hơn về thông số thiết bị được hiển thị ở giao diện này. Admin có thể xem nhanh các số liệu cần thiết.
...
Hình \ref{assign-admin} là giao diện minh họa giao diện phân công, cho mượn thiết bị, hỗ trợ admin trong việc quản lý xem người dùng nào đang phụ trách thiết bị.
...
Hình \ref{records-admin} là giao diện quản lý dữ liệu các phiên đo tại các buổi khám. Liên kết tới dữ liệu được lưu tại đây, admin bác sĩ có thể sử dụng cho việc phân tích sau này.
...
Hình \ref{graph} là giao diện thể hiện đồ thị chỉ số tim mạch thu thập được tự các dữ liệu phiên đo. Trong đó ECG, PPG, và PCG là các thuật ngữ liên quan đến các phương pháp đo tín hiệu sinh lý của cơ thể, thường được sử dụng trong y học và khoa học để giám sát sức khỏe. ECG (Electrocardiogram) - Điện tâm đồ là đường màu xanh lá, mô tả tín hiệu đo hoạt động điện của tim theo thời gian. PPG (Photoplethysmography) - Quang phổ thể tích là đường màu xanh dương, mổ tả sự thay đổi thể tích máu trong mạch máu bằng cách sử dụng ánh sáng. Và PCG (Phonocardiography) - Âm thanh tim, đường màu đỏ, mổ tả tín hiệu ghi lại âm thanh được tạo ra bởi hoạt động của tim, thường gọi là âm thanh tim.
\begin{figure}[H]
	\centering
...
Hình \ref{schedules-admin} là giao diện giúp admin có thể kiểm thử, thêm, sửa, xóa thông tin đặt lịch, từ đó có thể lấy dữ liệu để thống kê cho các mục đích sau này.
\begin{figure}[H]
	\centering
...
Hình \ref{schedule-info-admin} là giao diện trang thông tin đặt lịch, hỗ trợ người dùng có thể xem chi tiết tên bác sĩ, bệnh nhân, thời gian hẹn, giao diện này cũng sẽ dùng cho bệnh nhân và bác sĩ.
...
Hình \ref{chat-admin} là giao diện trang nhắn tin hỗ trợ admin có thể nhắn tin, tạo nhóm với người dùng khác để dễ dàng trong việc trao đổi, chia sẻ thông tin, hướng dẫn người dùng khác sử dụng hệ thống hoặc các chức vụ khác. 
...
Hình \ref{account-admin} là giao diện trang quản lý tài khoản, thông tin tài khoản, mật khẩu được lưu trữ tại đây, admin nói riêng hay người dùng nói chung có thể xem hoặc thay đổi thông tin về tài khoản của mình.
...
Hình \ref{schedule-with-doctor} là giao diện phục vụ mục đích: bệnh nhân có thể chọn bác sĩ mà bệnh nhân muốn đặt lịch. Sau đó chọn ngày giờ phù hợp để đặt lịch hẹn. Từ đó có thể dễ dàng kiểm tra lịch hẹn của mình, lựa chọn lịch hẹn phù hợp.
...
Hình \ref{noti-patient} minh họa giao diện thông báo để bệnh nhân có thể xem thông báo từ bác sĩ, thông báo từ hệ thống để biết xem mình đã được chấp nhận lịch hẹn hay chưa. Giao diện này cũng sẽ giống đối với bác sĩ.
...
Hình \ref{patient-list} là giao diện trang danh sách bệnh nhân hỗ trợ bác sĩ có thể xem danh sách bệnh nhân của mình, thông tin chi tiết của bệnh nhân.
...
Hình \ref{schdule-doctor} là giao diện trang quản lý lịch hẹn của bác sĩ để bác sĩ có thể xem thông tin chi tiết của lịch hẹn. Kiểm tra xem lịch làm việc của mình ngày hôm nay.
...
Hình \ref{review} là giao diện duyệt lịch hẹn. Các bác sĩ có thể chấp nhận hoặc từ chối lịch hẹn của bệnh nhân, sau đó thông báo sẽ được gửi đến bệnh nhân để họ có thể sắp xếp công việc của mình phù hợp cho việc khám bệnh.
...
Hình \ref{accepted} là giao diện sau khi bác sĩ chấp nhận lịch hẹn của bệnh nhân. Một pop up tinh gọn sẽ hiện lên để thông báo việc xác nhận lịch hẹn đã thành công!
...
\begin{figure}[H]
	...
	\caption[Sơ đồ tuần tự API chỉnh sửa thông tin chi tiết cho thiết bị y tế cụ thể]{\bfseries \fontsize{12pt}{0pt}\selectfont Sơ đồ tuần tự API cập nhật thông tin chi tiết cho thiết bị y tế cụ thể}
