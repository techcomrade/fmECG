...
export class ScheduleControllerClient {
    ...
    createScheduleByDoctor(body: ScheduleRequest): Promise<boolean> {
        let url_ = this.baseUrl + "/schedules/create-by-doctor";
        url_ = url_.replace(/[?&]$/, "");
    ...
    createScheduleByPatient(body: ScheduleRequest): Promise<boolean> {
        let url_ = this.baseUrl + "/schedules/create-by-patient";
        url_ = url_.replace(/[?&]$/, "");
...
export class DeviceControllerClient {
     ...
     * @return successful
     */
    getDeviceByUserId(): Promise<DeviceResponse[]> {
        let url_ = this.baseUrl + "/device/data/user";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetDeviceByUserId(_response);
        });
    }

    protected processGetDeviceByUserId(response: Response): Promise<DeviceResponse[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class RecordControllerClient {
     ...
     * @return Successful
     */
    createRecord(file: FileParameter | undefined, data: RecordRequest | undefined): Promise<boolean> {
...
export class DeviceRequest implements IDeviceRequest {
    ...
    status_id!: number;
    /** Start time when the user borrows the device */
    start_time!: number;
    /** End time when the user borrows the device */
    end_time!: number;
