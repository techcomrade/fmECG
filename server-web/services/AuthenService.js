...
class AuthenService extends CommonService {
  async login(account) {
    try{
      const accountData = await AccountRepository.getAccountByEmail(account.email);
      if(accountData?.dataValues){
        const result = bcrypt.compare(account.password, accountData.dataValues.password)
        if(!result) return result;
        var token = {
          id: uuidv4(),
          account_id: accountData.dataValues.id,
          access_token: TokenService.renderToken(accountData.dataValues.id, 60),
          refresh_token: TokenService.renderToken(accountData.dataValues.id, 120),
